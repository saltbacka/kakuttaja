...
define("CACHE_FOLDER", getcwd() . "/.cache");
define("EARLY_RETURN_CACHE", true);
define("DELETE_REMOVED_CACHE", false);
...
// 4. Remove all non alphanumeric characters and underscores
$cleanfilename = preg_replace("/[^A-Za-z0-9_]/", "", str_replace("/", "_", str_replace("_", "", preg_replace("/\." . $fileinfo['extension'] . "$/", "", trim($filename, "/.")))));

$cachefilename = $cleanfilename . "." . md5($file . $qs) . "." . $fileinfo["extension"];

$cachefile = CACHE_FOLDER . "/" . $cachefilename;
$cachefiletype = mime_content_type($cachefile);

$perf["vars_set"] = microtime(true) - $start;

if(!file_exists($file)) {
  http_response_code(404);
  if (DELETE_REMOVED_CACHE) {
    $oldcachefiles = preg_grep("~^" . $cleanfilename . "\.[a-f0-9]{32}\." . $fileinfo["extension"] . "$~", scandir(CACHE_FOLDER));
    foreach ($oldcachefiles as $v) {
      unlink(CACHE_FOLDER . "/" . $v);
    }
  }
  exit;
}

if(EARLY_RETURN_CACHE && file_exists($cachefile)) {
  ob_start();
  $cachefp = fopen($cachefile, "rb");
  header("Content-Type: $cachefiletype");
  header("Content-Lenght: " . filesize($cachefile));
  fpassthru($cachefp);
  fclose($cachefp);
  ob_end_flush();
  $perf["cache_sent"] = microtime() - $start;
  $sent = true;
}

if(!file_exists(CACHE_FOLDER)) {
  if(!mkdir(CACHE_FOLDER, 0700)) {
    http_response_code(500);
    die("Cannot create cache folder: " . CACHE_FOLDER);
  }
}
...
// echo "\$qs: $qs\n";
// echo "\$cleanfilename: $cleanfilename\n";
// echo "\$cachefilename: $cachefilename\n";
// echo "\$cachefile: $cachefile\n";
// echo "\$cachefiletype: $cachefiletype\n";
// foreach($args as $k => $v) echo "\$args[$k]: $v\n";
...
switch($fileinfo["extension"]) {
  case "html":
  case "htm":
    savefile(minify_html(file_get_contents($file)), $cachefile);
  case "css":
    savefile(minify_css(file_get_contents($file)), $cachefile);
    break;
  case "js":
    savefile(minify_js(file_get_contents($file)), $cachefile);
    break;
  case "jpg":
  case "jpeg":
  case "gif":
  case "png":
    compress_image($file, $cachefile, $args[0] ? $args[0] : 75);
    break;
}

// TODO: If the file was saved we should pre-send it before saving
